...
def transcribe(path: Path, vid: str, member: str, model: str,
        ...
        try:
            with path.open("rb") as stream:
                response = requests.post(
                    ...
                    headers=headers, data=stream, timeout=900)
            ...
            return {
                "status": "ok", "video_id": vid, "source_member": member,
    ...
    return {"status": "error", "video_id": vid, "source_member": member,
            "provider": "deepgram", "model": model, "language": language,
            "error": repr(last)}
...
def main() -> None:
    ...
    parser.add_argument("--model", default=os.environ.get("DEEPGRAM_MODEL") or "nova-3")
    parser.add_argument("--language", default=os.environ.get("DEEPGRAM_LANGUAGE") or "vi")
    ...
    def run(item: tuple[str, str]) -> dict:
        ...
        with tempfile.TemporaryDirectory(prefix=f"deepgram-{vid}-") as temp:
            ...
            try:
                ...
                return {"status": "error", "video_id": vid, "source_member": member,
                        "provider": "deepgram", "model": args.model,
                        "language": args.language, "error": repr(error)}
